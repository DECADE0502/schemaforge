...
2. 电源引脚(VCC/VDD)是否在顶部或左侧，接地(GND)是否在底部？
3. 输入引脚是否在左侧，输出引脚是否在右侧？
4. 总线信号(如 D0-D7)是否连续排列？
5. 双向信号(如 SDA/SCL)的位置是否方便连线？

**Pin + Body 分离规则 (关键):**
6. 每个引脚的连接点(pin endpoint)必须远离 IC 本体，引线(lead)要足够长
7. 不同侧的引脚不应出现在同一方位（如两个输出引脚不应一个在右一个在上）
8. 接地引脚(GND)必须在底部，画原理图时接地符号直接画在 pin 末端下方
9. 电源引脚(VIN/VCC)放在左侧或顶部，原理图中用电源标号而非长走线连接
10. 对于电源转换器(Buck/LDO/Boost): VIN/EN 必须在左侧, VOUT/SW/BST 必须在右侧, GND/FB 在底部
...
1. IC 矩形是否正确显示了器件名称？
2. 每个引脚是否正确标注了名称和编号？
3. 引脚方位是否符合 EDA 习惯（电源上/左、地下、输入左、输出右）？
4. 反相引脚(~开头)是否有反相标记？
5. 整体布局是否清晰、间距合适？
6. **引脚引线(lead)是否足够长？** 连接点应远离 IC 本体，留出走线空间（lead 至少为 body 尺寸的 1/4）
7. **GND 引脚是否在底部？** 画原理图时需要在 GND pin 下方直接画接地符号
8. **不同功能的引脚是否分在不同侧？** VIN/EN 左侧, VOUT/SW 右侧, GND/FB 底部
