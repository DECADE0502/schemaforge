...
class DeviceRetriever:
    ...
    def _fetch_candidates(
        ...
        策略：
        1. 有分类 → 按分类搜 + 按 "other" 搜（用户导入器件通常是 other）
        2. 有关键字 → 按关键字搜
        3. 合并去重
        """
        seen: set[str] = set()
        ...
        def _merge(devices: list[DeviceModel]) -> None:
            for d in devices:
        ...
        if category:
            # 按标准分类搜
            _merge(self._store.search_devices(category=category))
            # 用户导入器件通常 category="other"，也纳入候选
            if category != "other":
                _merge(self._store.search_devices(category="other"))

        if query:
            _merge(self._store.search_devices(query=query))
